...
export class TabManager {
  private readonly tabs: TabRecord[] = [
    { id: HOME_ID, kind: 'home', view: null, title: 'ReveLith' },
  ]
  private activeId: string = HOME_ID
  /** Shell HTML modals must sit above native WebContentsViews. */
  ...
  openDocsTab(openPath?: string, options?: { newBlank?: boolean }): string {
    ...
    this.tabs.push({
      ...
      kind: 'docs',
      view,
      title: openPath ? basename(openPath) : this.untitled('docs', 'ReveLith Docs'),
      filePath: openPath,
    })
    this.activateTab(id)
    return id
  ...
  activateTab(id: string): void {
    ...
    if (!target) return
    for (const t of this.tabs) t.view?.setVisible(t.id === id && !this.shellOverlayOpen)
  ...
  /**
   * Native WebContentsViews always paint above the shell renderer, so hide the
   * active view while a shell-owned modal (such as Settings) is open.
   */
  setShellOverlayOpen(open: boolean): void {
