...
import { configureMetricsCache, familyVerticalMetrics } from '@revelith/font-metrics'
import { createI18n, getUiLang, normalizeLang, setUiLang } from '@revelith/i18n'
...
const testExportDir = process.env.REVELITH_TEST_EXPORT_DIR || null
...
/** office/pdf formats get text extracted via @revelith/file-parse; images skip extraction and go multimodal (files:read-image) */
...
/** parse an attachment to text via @revelith/file-parse (docx/pdf/pptx/xlsx/plain text) */
...
// implementations live in @revelith/ai-provider, shared with apps/sheets.
...
export function registerAiIpc(): void {
  ...
  ipcMain.handle('ai:set-settings', (_event, settings: AiSettings) => {
    ...
    // Settings can be changed in the Shell while Docs/Sheets/Slides are open
    // in separate WebContentsViews. Push the authoritative settings immediately
    // so their model pickers never keep a stale provider/model in memory.
    for (const contents of webContents.getAllWebContents()) {
      if (!contents.isDestroyed()) contents.send('ai:settings-changed', settings)
    }
  })
    ...
    try {
      ...
      const checks = await Promise.all(
        result.images.map(async (image) => {
          const response = await fetchRemoteImage(image.imageUrl)
