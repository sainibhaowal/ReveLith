...
function fixtureDoc(): JsonNode[] {
  return [
    ...
    para([text('ReveLith intro,'), text('ReveLith is great', [{ type: 'bold' }])], {
      docxIndex: 1,
    }),
...
describe('replaceAllText', () => {
  it('replaces every occurrence and keeps marks', () => {
    const editor = createEditor(fixtureDoc())
    const outcome = executeCommands(editor, {
      commands: [{ replaceAllText: { containsText: 'ReveLith', replaceText: 'ReveLith' } }],
    })
    expect(outcome.ok).toBe(true)
    expect(outcome.results[0].detail).toBe('共替换 2 处')
    const block = editor.state.doc.child(1)
    expect(block.textContent).toBe('ReveLith intro,ReveLith is great')
  ...
  it('matchCase: sensitive by default, insensitive when false', () => {
    ...
    const strict = executeCommands(editor, {
      commands: [{ replaceAllText: { containsText: 'revelith', replaceText: 'X' } }],
    })
    expect(strict.results[0].changed).toBe(0)
    expect(editor.state.doc.child(1).textContent).toContain('ReveLith')

    const loose = executeCommands(editor, {
      commands: [
        { replaceAllText: { containsText: 'revelith', replaceText: 'X', matchCase: false } },
      ],
    })
...
describe('envelope validation', () => {
  it('unknown command rejects the whole envelope with no doc change', () => {
    ...
    const outcome = executeCommands(editor, {
      commands: [
        { replaceAllText: { containsText: 'ReveLith', replaceText: 'X' } },
        { insertTable: { rows: 2 } },
      ],
    } as unknown as CommandEnvelope)
    expect(outcome.ok).toBe(false)
  ...
  it('a multi-command round undoes in a single step', () => {
    ...
    const outcome = executeCommands(editor, {
      commands: [
        ...
        { replaceAllText: { containsText: 'ReveLith', replaceText: 'ReveLith' } },
