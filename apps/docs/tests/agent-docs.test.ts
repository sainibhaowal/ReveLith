...
import { AgentLoop, type AgentStreamCallbacks, type AgentTransport } from '@revelith/agent-core'
...
const fixture = () => [
  ...
  para('ReveLith is an AI office suite.'),
...
describe('content read/write tools', () => {
  it('read_blocks returns the full restricted HTML', async () => {
    ...
    expect(exec.output).toContain('<p>ReveLith is an AI office suite.</p>')
  })
...
describe('web_search backend failures', () => {
  it("method 'error' surfaces as a tool error instead of '(no results)'", async () => {
    ...
    try {
      const exec = await executeTool(
        editor,
        { id: 't', name: 'web_search', input: { query: 'revelith' } },
        NUM_IDS,
      )
      expect(exec.isError).toBe(true)
