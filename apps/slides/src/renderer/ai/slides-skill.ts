import type { AgentSkill, ToolDisplay } from '@revelith/agent-core'
import type {
  GroupRenderNode,
  PictureRenderNode,
  RenderNode,
...
export interface DeckAccess {
  ...
  /** Whether cloud single-page generation is available (kill switch + account login state) */
  isCloudPageGenEnabled?(): Promise<boolean>
  /**
   * Cloud single-page generation (account slide_generate), used by generate_deck's self-driven
...
const AGENT_SYSTEM_PROMPT = `You are the AI assistant inside Revelith Slides (a slide editor), helping users improve and generate presentations.
...
/** Render a usable deck locally for every configured AI provider. */
async function generateNativeDeck(
  access: DeckAccess,
  pages: Array<Record<string, unknown>>,
  insertMode: 'replace' | 'append',
): Promise<{ ok: boolean; done: number; error?: string }> {
  let all = access.getSlides()
  let start = all.length
  if (insertMode === 'replace') {
    const fresh = await window.slidesApi.newBlank(access.fitWidthPx)
    if (!fresh || !('slides' in fresh) || !Array.isArray(fresh.slides))
      return { ok: false, done: 0, error: 'Could not create a blank presentation.' }
    all = fresh.slides
    start = 0
    access.applyDeck(all, 0)
  }
...
function nativeSlideCopy(brief: string, title: string): string {
  ...
  if (/\bAI\b/i.test(clean))
    return 'AI turns project data into clear decisions.\n• Celebrate measurable progress\n• Identify risks early\n• Focus teams on the next best action'
  return `${title}\n• Clear progress updates\n• Key decisions and priorities\n• Next steps for the team`
}

/** Rebuild one existing page without a cloud page-generation provider. */
async function regenerateNativeSlide(
  access: DeckAccess,
  slideIndex: number,
  title: string,
  brief: string,
  ...
  if (!slide) return { ok: false, error: 'Slide does not exist.' }
  // Clear only top-level elements; deleting a group removes all of its children.
  for (const node of slide.nodes) {
    const updated = await window.slidesApi.deleteElement({ slideIndex, sourceId: node.sourceId })
    if (updated) access.applySlide(slideIndex, updated)
  }
  ...
  const cleanBrief = nativeSlideCopy(brief, title)
...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'generate_image',
    description:
      'AI image generation/editing (ReveLith). Text-to-image, or pass referenceImageUrls for image editing; returns an image URL. NEW imagery: insert with insert_web_image. Editing an EXISTING slide picture (background removal/upscaling/etc.): swap it in place with replace_image : do not insert a duplicate. Use for custom illustrations/icons/backgrounds, style-consistent imagery; for real photos/screenshots still use image_search.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    name: 'analyze_media',
    description:
      'Analyze media content (ReveLith): understand images/audio/video. Pass media URLs (or local file paths) and analysis requirements; returns analysis text. Video supports extracting key points, structure, and time ranges : good for turning user material into usable deck content.',
    inputSchema: {
      type: 'object',
      properties: {
        mediaUrls: {
...
function blockScratchBuild(
  ...
  // The former hosted HTML generator is deliberately disabled.  Native creation must remain
  // available to every selected provider, including local models.
  void toolName
  void slides
  void state
  return null
...
async function executeTool(
  ...
  switch (call.name) {
    ...
    case 'regenerate_slide': {
      ...
      if (!cloudPageGeneration) {
        const native = await regenerateNativeSlide(access, idx, title, brief)
        if (!native.ok) return fail(t('aiFailRegen'), native.error ?? 'Native page rebuild failed.')
        return {
          output: `Rebuilt page ${idx + 1} locally with editable native elements. No cloud page-generation provider was used.`,
          mutated: true,
      ...
      if (!marker)
        return fail(
          t('aiFailRegen'),
          `Page generation failed (2 attempts): ${lastErr}. Do not keep calling regenerate_slide in a loop. Instead, make the requested changes in place with execute_slide_script / set_element_* (group children are editable too), or ask the user to retry. The page was not modified.`,
        )
    ...
    case 'generate_deck': {
      ...
      // Native fallback: local/BYOK providers never require a cloud slide account.
      // The provider produces the outline above; ReveLith creates editable PPTX
      // elements itself. This also makes blank slides impossible on a successful run.
      if (!cloudPageGeneration) {
        ...
        access.onProgress?.({
          stage: 'done',
          total: native.done,
          summary: `Created ${native.done} editable slide(s) locally with the selected provider.`,
        })
        return {
          output: `Created ${native.done} editable slide(s) locally. No cloud deck-generation provider was used.`,
          mutated: true,
      ...
      // The cloud service (account slide_generate) writes each page's HTML and converts it to a
