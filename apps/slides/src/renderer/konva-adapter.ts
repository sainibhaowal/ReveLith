...
export function fillImageUrl(fill: RenderFill | undefined): string | undefined {
...
export function layoutGlyphs(text: RenderTextLayout | undefined): GlyphDraw[] {
  if (!text || !Array.isArray(text.lines)) return []
  const out: GlyphDraw[] = []
  for (const line of text.lines) {
    if (line && Array.isArray(line.runs)) {
      for (const run of line.runs) {
        if (run) out.push(glyphToDraw(run))
      }
    }
  }
  return out
