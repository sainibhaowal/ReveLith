...
function NodeView({
  ...
  const { t } = useI18n()
  const box = node.box ?? { x: 0, y: 0, w: 0, h: 0, rotationDeg: 0 }
  const bx = Number.isFinite(box.x) ? box.x : 0
  const by = Number.isFinite(box.y) ? box.y : 0
  const bw = Number.isFinite(box.w) ? box.w : 0
  const bh = Number.isFinite(box.h) ? box.h : 0
  const rot = Number.isFinite(box.rotationDeg) ? box.rotationDeg : 0
  ...
  useLayoutEffect(() => {
    ...
    if (g && transformingRef.current && (prev.w !== bw || prev.h !== bh)) {
      if (bw > 0) g.scaleX(g.scaleX() * (prev.w / bw))
      if (bh > 0) g.scaleY(g.scaleY() * (prev.h / bh))
    }
  ...
  const flipOffX = box.flipH ? bw : 0
  const flipOffY = box.flipV ? bh : 0
