...
export function registerSlidesIpc(): void {
  ...
  // ── Cloud single-page generation (account slide_generate): brief → cloud HTML+conversion → one-slide
  ...
  // converting. Enabled when account is logged in; REVELITH_CLOUD_SLIDE=0 is the kill switch.
  // All providers use the native ReveLith deck renderer. No separate cloud
  // slide-generation account or API key is required.
  ...
  ipcMain.handle(
    ...
    async (
      ...
      void op
      return { ok: false, error: 'Cloud slide generation is disabled; use the native renderer.' }
    },
  )
