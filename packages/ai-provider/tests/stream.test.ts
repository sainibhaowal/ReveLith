...
describe('streamForProvider: anthropic', () => {
  ...
  it('never sends an empty assistant content array when history has edits-only replies', async () => {
    ...
    // on Anthropic (revelith#12 / #22 class of multi-turn failures).
    const fetchMock = vi
      .fn()
      .mockResolvedValue(
        okResponse(
...
describe('streamForProvider: 200 + non-stream JSON instead of SSE', () => {
  const creditsNotice =
    'Your ReveLith credits have been exhausted. Please visit https://www.revelith.ai/pricing to purchase more credits.'
      ...
      vi.fn().mockResolvedValue(
        json({
          candidates: [
            {
              content: {
                parts: [{ text: 'Out of quota, visit https://www.revelith.ai/pricing to top up.' }],
              },
            },
          ],
