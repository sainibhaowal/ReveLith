...
export async function chatForProvider(
  ...
  config = await resolveLocalModelConfig(provider, config, signal)
    ...
    switch (provider) {
      ...
      default:
        return config.baseUrl
          ? chatOpenAiCompatible(wd, config.baseUrl, config, system, user)
          : Promise.resolve({
              ok: false as const,
              error: `Provider ${provider} requires a Base URL or a built-in adapter`,
            })
    }
  })
}
